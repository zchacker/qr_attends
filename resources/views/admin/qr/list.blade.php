...
<div class="content">
    ...
    <div class="py-6">
                    ...
                    <div class="px-4">
                        <div class="flex justify-between sm:flex sm:items-center">
                            ...
                            <div class="w-1/3 flex gap-4 justify-end">                               
                                <div >
                                    <a href="{{ route('admin.qr.select_type') }}" class="normal_button"> {{__('add_qr')}} </a>
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                <table class="table">
                                    <thead class="">
                                        <tr>
                                            ...
                                            <th scope="col" class="py-3 px-6"> {{__('qr_code')}} </th>
                                            <th scope="col" class="py-3 px-6"> {{__('type')}} </th>
                                            <th scope="col" class="py-3 px-6"> {{__('name')}} </th>
                                            <th scope="col" class="py-3 px-6"> {{__('visits')}} </th>
                                    ...
                                    <tbody class="table_body">
                                        ...
                                        <tr data-href="" class="clickable-row cursor-pointer hover:bg-gray-200">
                                            @php 
                                                $qr = base64_encode( QrCode::size(50)->format('png')->generate($code->data) );
                                            @endphp
                                            <td class="whitespace-nowrap py-4 px-3 text-sm text-gray-500"> {{ $code->id }} </td>
                                            <td class="whitespace-nowrap py-4 px-3 text-sm text-gray-500"> 
                                                <img src="data:image/png;base64, {{ $qr }}" alt="">
                                            </td>
                                            <td class="whitespace-nowrap py-4 px-3 text-sm text-gray-500"> {{ __($code->type) }} </td>
                                            <td class="whitespace-nowrap py-4 px-3 text-sm text-gray-500"> {{ $code->name }} </td>
                                            <td class="whitespace-nowrap py-4 px-3 text-sm text-gray-500"> {{ $code->visits }} </td>
                                            <td class="whitespace-nowrap py-4 px-3 text-sm text-gray-500"> {{ $code->created_at }} </td>
                                            <td class="relative flex gap-4 items-center justify-between whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6 md:pr-0">
                                                ...
                                                @php 
                                                    $qr = base64_encode( QrCode::size(30)->format('png')->generate(Request::url()) );
                                                @endphp
                                                --}}

                                                {{--
                                                <a href="data:image/png;base64, {{ $qr }}" id="download" download="qr_code.png">
                                                ...
                                                <a href="{{ route('admin.qr.download' , $code->id) }}" id="download" download="qr_code.png">
                                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="h-6 w-6" id="qr-code">
                                                        <path d="M17 3h-1c-.6 0-1 .4-1 1s.4 1 1 1h1c1.1 0 2 .9 2 2v1.1c0 .6.4 1 1 1s1-.4 1-1V7c0-2.2-1.8-4-4-4zm3 12c-.6 0-1 .4-1 1v1c0 1.1-.9 2-2 2h-1c-.6 0-1 .4-1 1s.4 1 1 1h1c2.2 0 4-1.8 4-4v-1c0-.6-.4-1-1-1zM8 19H7c-1.1 0-2-.9-2-2v-1c0-.6-.4-1-1-1s-1 .4-1 1v1c0 2.2 1.8 4 4 4h1c.6 0 1-.4 1-1s-.4-1-1-1zM4 9c.6 0 1-.4 1-1V7c0-1.1.9-2 2-2h1c.6 0 1-.4 1-1s-.4-1-1-1H7C4.8 3 3 4.8 3 7v1c0 .5.4 1 1 1z"></path>
                                                        <path d="M12 8c0-1.1-.9-2-2-2H8c-1.1 0-2 .9-2 2v2c0 1.1.9 2 2 2h2c1.1 0 2-.9 2-2V8zm-4 2V8h2v2H8zm10 6v-2c0-1.1-.9-2-2-2h-2c-1.1 0-2 .9-2 2v2c0 1.1.9 2 2 2h2c1.1 0 2-.9 2-2zm-4-2h2v2h-2v-2zm0-3h1c.6 0 1-.4 1-1V8h1c.6 0 1-.4 1-1s-.4-1-1-1h-2c-.6 0-1 .4-1 1v2c-.6 0-1 .4-1 1s.4 1 1 1zm-4 2H7c-.6 0-1 .4-1 1s.4 1 1 1h2v2c0 .6.4 1 1 1s1-.4 1-1v-3c0-.6-.4-1-1-1z"></path>
                                                    </svg>
                                                ...
                                                <form action="{{ route('admin.qr.delete', $code->id) }}" method="POST" onsubmit="return confirmDelete()">
                                                    @method('DELETE')
                                                    @csrf
        ...
        <div class="text-left mt-10" dir="rtl">
            {{ $codes->onEachSide(5)->links('pagination::tailwind') }}
        </div>
